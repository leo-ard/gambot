...
(define http-server-start!
  (lambda (hs)
    (let ((server-port
           (open-tcp-server
            (list server-address: '#u8(0 0 0 0) ; It's better to choose 0.0.0.0 for docker compatibility
...
(define reply
  (lambda (thunk #!optional (content-type "text/html"))
    (let* ((request
            (current-request))
           (connection
              ...
              (let ((message
                ...
                (print
                 ...
                 (list version " 200 OK" eol
                       ...
                       "Content-Type: " content-type "; charset=ISO-8859-1" eol
...
(define reply-json
  (lambda (json)
    (reply (lambda () (print json)) "application/json")))

(define reply-text
  (lambda (text #!optional (type "text/plain"))
    (reply (lambda () (print text)) type)))
